...
exports.handler = async function (event, context) {
	...
	const response = await documentClient.query(queryParmas).promise();

	const sosId = response.Items[0].sosId;
	let errorCount = 0;
	let sosIdIncrement = 1;

	while (errorCount < 20) {
		let business: any;

		try {
			business = await getDetails(sosId + sosIdIncrement);
		}
		catch (e) {
			console.log('Error when trying to get details for', sosId + sosIdIncrement, e);
			errorCount++;
			continue;
		}
		business.id = uuidv4();
		...
		if (business.title === 'No CUR/IP/NON/UNF EN/DBA for this entity') {
			console.log('No business found for', sosId + sosIdIncrement);
			errorCount++;
		}
		else {
			errorCount = 0;
